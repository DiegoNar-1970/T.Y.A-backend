...
class CustomerModel {
    ...
    static async findByCcAndCreate(customer) {
        ...
        try {
            //se consulta si esxiste el cliente en la base de datos
            const { rows } = await configDb_1.connection.query(`
                SELECT * FROM customer WHERE document = $1
            `, [customer.document]);
            //si no existe se crea y se devuelve
            if (rows.length === 0) {
                const response = await configDb_1.connection.query(`
    ...
    static async updatePartialCustomer(data) {
        try {
            ...
            `;
            const result = await configDb_1.connection.query(query, [...values, data.id]);
            return result.rows[0];
        }
        catch (error) {
            if (error instanceof Error) {
