...
class ContractModel {
    ...
    static async getById(id) {
        try {
            const { rows } = await configDb_1.connection.query(`SELECT * FROM contract WHERE id = $1`, [id]);
            if (rows.length === 0) {
                throw new AppError_1.AppError(`
            EL contrato no existe`, 'NOT_FOUND', 400);
            }
            ;
            if (rows[0].asigned === true) {
                throw new AppError_1.AppError(`No puedes firmar un contrato Mas de 1 vez`, 'EXISTING_RESOURCE', 400);
            }
            return rows[0];
        }
        catch (error) {
    ...
    static async create(contract) {
        const { id_info_contract, id_employee, num_contract, observation, path } = contract;
        try {
            const result = await configDb_1.connection.query(`
    ...
    static async ifExist(id) {
        try {
            const { rows } = await configDb_1.connection.query('SELECT * FROM contract WHERE num_contract = $1', [id]);
            if (rows.length !== 0) {
                throw new AppError_1.AppError(`Ya existe un contrato con el número ${id}`, 'ID_DUPLICATE', 400);
            }
            ...
            throw new AppError_1.AppError('Error al verificar el contrato', 'CHECK_ERROR', 500);
        }
    }
    static async update(id, data) {
        const { id_info_contract, id_employee, num_contract, observation, path } = data;
        try {
            const result = await configDb_1.connection.query(`
          UPDATE contract SET
    ...
    static async delete(id) {
        try {
            const { rowCount } = await configDb_1.connection.query(`DELETE FROM contract WHERE id = $1 RETURNING *`, [id]);
    ...
    static async getByRadicado(radicado) {
        try {
            ...
            SELECT 
              ...
              `, [radicado]);
            if (rows.length === 0) {
                throw new AppError_1.AppError(`No se encontro nigun contrato con el id ${radicado}`, 'ID_NOT_REGISTRED', 400);
            }
            return rows[0];
        }
        catch (error) {
    ...
    static async getByNumContract(numContract) {
        ...
        SELECT 
              ...
              `, [numContract]);
            if (rows.length === 0) {
                throw new AppError_1.AppError(`No se encontro nigun contrato con el id ${numContract}`, 'ID_NOT_REGISTRED', 400);
            }
            return rows[0];
        }
        catch (error) {
    ...
    static async countContracts() {
        try {
            const { rows } = await configDb_1.connection.query('select count(*) from contract');
            return rows[0];
        }
        catch (err) {
    ...
    static async countContAsigned() {
        try {
            ...
            FROM contract; `);
            if (rows.length === 0) {
                throw new AppError_1.AppError('Sin registros contratos');
            }
            return rows[0];
        }
        catch (error) {
    ...
    static async countContWithoutAsigned() {
        try {
          ...
          FROM contract;`);
            if (rows.length === 0) {
                throw new AppError_1.AppError('Sin registros de contratos');
            }
            return rows[0];
        }
        catch (error) {
    ...
    `;
            ...
            const result = await configDb_1.connection.query(query, values);
            if (result.rows.length === 0) {
                throw new AppError_1.AppError(`No se encontro nigun registro de este recurso`, 'ID_NOT_REGISTRED', 400);
            }
            return result.rows;
        }
        catch (error) {
